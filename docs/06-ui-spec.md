...
| saved (M9-T3, fixed M23-T3; **promoted M35-T2**) | **a tick plus the word `Saved`, ~3 s** — the item's `title`, beside the composited tick. **Measured 2026-08-06:** present within 0.4 s, still there at 2.8 s, gone by 3.2 s, and **~32 % of the item's area changes** against the armed baseline where the tick alone changed **4 %**. The word is the whole point: this is the only confirmation a replay save has when banners are suppressed, which is *whenever* replay is armed. Empty the rest of the time — a permanent word in the menu bar would push every other item along for good (`StatusItemPolicy.title`). ⚠️ The item widens while shown, so neighbours shift for those 3 s; accepted knowingly (Franco, 2026-08-06), and the tick alone already did it | ⚠️ **had never rendered**: it was a second `Image` in the label's `HStack`, and a MenuBarExtra draws only the first (measured, docs/07). Now composited like the badge, meter and clock. Raised by a replay save, and by a take that stops **while replay is armed** — the case that was otherwise silent on every channel, since an armed stream keeps the display captured and macOS suppresses banners then (M5-T5). An ordinary stop does not flash: its banner works, and two confirmations of one event is noise. Not raised for `.writeFailed`, whose notice is a problem to read rather than a tick to glance at |
